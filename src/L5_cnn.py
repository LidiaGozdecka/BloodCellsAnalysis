...
# Pobieramy surowe indeksy klas dla wszystkich próbek treningowych
train_labels = train_gen.classes

# Obliczamy zbalansowane wagi klas przy użyciu scikit-learn
...
# Keras wymaga słownika w formacie {indeks_klasy: wartość_wagi}
class_weight_dict = dict(zip(unique_classes, class_weights_array))

print("Wyznaczone wagi dla poszczególnych indeksów klas:")
for cls_idx, weight in class_weight_dict.items():
    # Pobieramy tekstową nazwę klasy dla lepszej czytelności logów
...
history = model.fit(
    ...
    class_weight=class_weight_dict,  # o tu dodaje wagi wiec juz spryciarz nie bedzie przewidywal w kolko neutrofili :DD
    verbose=2
)
...
plt.legend()
# --- ZAMIANA PLT.SHOW() W L5 ---
plt.savefig(os.path.join(script_dir, "krzywe_uczenia_cnn_l5.png"), bbox_inches='tight', dpi=150)
plt.close()
print("[INFO] Wykres krzywych uczenia CNN został zapisany jako 'krzywe_uczenia_cnn_l5.png'.")
